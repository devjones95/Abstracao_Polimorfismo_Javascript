...
class Imovel {
    ...
    descrever() {
        throw new Error("Este método devser implementado por subclasses");
    }
}
...
class Apartamento extends Imovel {
    ...
    // Implementação do método descrever
    descrever() {
        let descricao = `Apartamento localizado em ${this.endereco}, no ${this.numeroDoAndar}º andar, de tamanho ${this.tamanho}m²`;
...
// Função para descrever um imóvel
function descreverImovel(imovel) {
    console.log(imovel.descrever());
}

// Criação de objetos (instâncias das classes)
const minhaCasa = new Casa("Rua dos Cedros, 122", 120, "verde", 3, true);
const meuApartamento = new Apartamento("Avenida Charles Schnneider, 909", 85, 7, true);

// Manipulação de Objetos e exibição das descrições usando polimorfismo
